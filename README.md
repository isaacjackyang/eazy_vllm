...
- `start_vllm_server.cmd`：啟動雙卡 `vLLM` 伺服器，預設模型是 `TeichAI/gemma-4-31B-it-Claude-Opus-Distill`。
- `start_vllm_server_openclaw.cmd`：以 `OpenClaw` 連線為前提啟動 `vLLM`，預設模型是 `TeichAI/gemma-4-31B-it-Claude-Opus-Distill`，並使用 `--max-model-len 96000`。
...
start_vllm_server_openclaw.cmd --dry-run
```
...
```bash
vllm serve "TeichAI/gemma-4-31B-it-Claude-Opus-Distill" \
...
```powershell
# 請將 your_ubuntu_username 替換成實際值；預設模型為 TeichAI/gemma-4-31B-it-Claude-Opus-Distill
wsl -d Ubuntu-22.04 -u your_ubuntu_username -- bash -lc "source ~/vllm-env/bin/activate && vllm serve 'TeichAI/gemma-4-31B-it-Claude-Opus-Distill' --host 0.0.0.0 --tensor-parallel-size 2 --max-model-len 96000 --gpu-memory-utilization 0.9 --port 8000"
```
...
```cmd
start_vllm_server.cmd "TeichAI/gemma-4-31B-it-Claude-Opus-Distill"
```
...
如果你就是要給 Windows 端的 `OpenClaw` 直接連線，現在也可以直接雙擊：

```cmd
start_vllm_server_openclaw.cmd
```
